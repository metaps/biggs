...
describe "ActiveRecord Class" do
  ...
  it "should include Biggs::ActiveRecordAdapter" do
    expect(FooBar.included_modules).to include(Biggs::ActiveRecordAdapter)
  end
  
  it "should set class value biggs_value_methods" do
    result = FooBar.class_eval("biggs_value_methods")
    expect(result).to be_kind_of(Hash)
    expect(result.size).to eq(0)
  end
  
  it "should set class value biggs_instance" do
    expect(FooBar.class_eval("biggs_instance")).to be_kind_of(Biggs::Formatter)
  end
  
  it "should respond to biggs" do
    expect(FooBar).to respond_to(:biggs)
  end
end
